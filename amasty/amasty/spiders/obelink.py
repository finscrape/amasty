...
class ObelinkSpider(XMLFeedSpider):
    name = 'obelink'
    ...
    start_urls = ['https://www.obelink.nl/media/sitemap/sitemap_obelink_nl_nl-2-1.xml','https://www.obelink.nl/media/sitemap/sitemap_obelink_nl_nl-2-2.xml','https://www.obelink.nl/media/sitemap/sitemap_obelink_nl_nl-2-3.xml','https://www.obelink.nl/media/sitemap/sitemap_obelink_nl_nl-2-4.xml']
    ...
    def next(self,response):
        check = response.xpath("//div[@class='product media']").get()
        if check:
        
            pd_name = response.xpath("//h1[@class='page-title']/descendant::text()").get()
            prc =response.xpath("//span[@class='price']/descendant::text()").get()
            urlc = response.url
            site = 'https://www.obelink.nl/'
            ean = response.xpath("//th[contains(text(),'EAN')]/following-sibling::td/text()").get()
            sku = response.xpath("//th[contains(text(),'Artikelnummer fabrikant')]/following-sibling::td/text()").get()
            Brand = response.xpath("//th[contains(text(),'Merk')]/following-sibling::td/text()").get()
            com = 'Obelink'
