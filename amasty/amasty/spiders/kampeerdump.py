...
class KampeerdumpSpider(XMLFeedSpider):
    name = 'kampeerdump'
    ...
    def start_requests(self):
        alc = ['https://www.kampeertotaal.nl/sitemap/sitemap-5-1.xml','https://www.kampeertotaal.nl/sitemap/sitemap-5-2.xml']
    
        for i in alc:
            yield SeleniumRequest(url=i,callback=self.parse)
    def parse(self,response):
        sleep(20)
        l = response.xpath("//span[contains(text(),'https')]/text()").getall()
        for i in l:
            
            self.all.append(i)
            yield scrapy.Request(url=i,callback=self.next,dont_filter=False)
    ...
    def next(self,response):
        check = response.xpath("//div[@class='product-info-main']").get()
        if check:
        
            pd_name = response.xpath("//meta[@name='title']/@content").get()
            prc =response.xpath("//meta[@itemprop='price']/@content").get()
            urlc = response.url
            site = 'https://www.kampeertotaal.nl/'
            ean = 'No value'
            sku = response.xpath("//th[text()='Art.nr.']/following-sibling::td/text()").get()
            Brand = 'None'
            com = 'KampeerTotal'
